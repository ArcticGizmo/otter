---
name: bump-version
description: Bump the Otter version and refresh the changelog. Determines the next version from the LAST GIT TAG (not the csproj value, which can drift), bumps src/Otter.csproj, backfills changelog sections for any tagged versions that were never documented, summarises every change since the last tag into a new CHANGELOG.md section, folds in the Unreleased section, clears Unreleased, and prints the new version plus its changelog entries. Use when the user says "bump version", "bump the version", "/bump-version", or wants to cut a new release entry.
---

# Bump Version

Bumps Otter to the next version and brings `CHANGELOG.md` up to date.

## Why the tag, not the csproj

The version in `src/Otter.csproj` and the `[Unreleased]` changelog section both drift —
sometimes a version gets tagged without the csproj or changelog catching up. The **last git
tag is the source of truth**. Always derive the next version from it.

## Steps

### 1. Gather everything in one batch

Run this single block first — it collects the tag, the new commits, and the dates for any
backfill, so you don't make repeated round-trips:
...
(e.g. `v0.2.0` → `v0.2.1`). Call it `$NEXT` (csproj wants it without the `v`, e.g. `0.2.1`).
...
would never notice (version bumps, release-pipeline tweaks, internal refactors). Collapse
related commits into the single *net* change they add up to (see "What to write" in step 4).

### 2. Bump the csproj

Edit the `<Version>` element in `src/Otter.csproj` to `$NEXT` (no `v` prefix). Leave
...
correct reverse-chronological slot, applying the same summarising rules below. Do every
missing tag, not just the latest.
...
- **Write for the end user, not the developer.** Describe what changed for someone *using*
  Otter. Mention implementation details only when they genuinely matter to the user (e.g.
  "existing settings carry over"). Internal refactors, file moves, and plumbing don't get a
  bullet at all.
- **Keep bullets snappy.** A few words to a short sentence, not a paragraph. Cut "now", "the
  ability to", "you can now". Prefer "Snooze from the tray" over "You can now snooze Otter from
  the tray".
- **Lead with a bold label where it helps.** The existing entries open feature bullets with a
  short bold lead-in (e.g. `**Live status preview** — …`); match that for notable features and
  use plain bullets for smaller changes.
...
Otter's changelog has a clean, plain voice — clear bullets that say what changed and why it
matters to the user, no marketing fluff and no forced jokes. Match the register of the
existing entries. Don't invent changes that didn't happen.

### 5. Report back

Print, plainly:

- The next version number (e.g. `v0.2.1`).
...
- Only the patch component is bumped by default. If the user wants a minor/major bump, they'll
  say so — follow their instruction instead of auto-incrementing patch.
- The changelog is embedded into the app (`Otter.CHANGELOG.md`) and rendered on the settings
  window's Changelog page, so keep it to the markdown subset that page understands: `##`/`###`
  headings, `-` bullets, `>` blockquotes, `---` rules, and inline `**bold**`/`*italic*`/links.
